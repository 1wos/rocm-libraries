...
"""
FMHA hardware and kernel specifications — single source of truth.

This file consolidates ALL architecture-dependent facts, tile constraints,
feature rules, tile tables, and profile definitions. When the FMHA kernel
code in CK changes, update THIS file — nothing else should need to change.
...
from symbol_map import canonical_bias, canonical_qscale
...
ARCH_DTYPES: Dict[str, List[str]] = {
    ...
    "gfx950": ["fp16", "bf16", "fp32", "fp8", "fp8bf16", "fp8fp32", "bf8", "mxfp8", "mxfp4"],
    "gfx1100": ["fp16", "bf16"],
    "gfx1201": ["fp16", "bf16"],
}
...
SUPPORTED_HDIMS: Dict[str, List[Tuple[int, int]]] = {
    "fp16": [
        (32, 32), (64, 64), (80, 96), (96, 128), (128, 128),
        (160, 160), (192, 128), (192, 192), (256, 256),
    ],
    "bf16": [
        (32, 32), (64, 64), (80, 96), (96, 128), (128, 128),
        (160, 160), (192, 128), (192, 192), (256, 256),
    ...
    "fp32": [
        (32, 32), (48, 48), (64, 64), (96, 128), (128, 128),
        (192, 192), (256, 256),
    ],
...
# LDS capacity per pipeline type (bytes)
LDS_LIMITS: Dict[str, int] = {
    "qr": 65536,
    "qr_async": 32768,
    "qr_async_trload": 32768,
    "qr_async_trload_v3": 131072,
    "qs": 65536,
}

# Warp configurations: (wm0, wn0, wk0) — MFMA instruction tile sizes
...
ELEMENT_SIZES: Dict[str, int] = {
    ...
    "fp8": 1, "fp8bf16": 1, "fp8fp32": 1, "bf8": 1,
    "mxfp8": 1, "mxfp4": 1,
}

# Block tile dimensions to sweep
VALID_BM0 = [16, 32, 64, 128, 256]
VALID_BN0 = [16, 32, 64, 96, 128, 192, 256]
VALID_BK0 = [16, 32, 64, 96, 128, 256]

# K0_MAX_SUBMAX_MAP from CK — maps hdim_q to bk0max
K0_MAX_SUBMAX_MAP: Dict[int, int] = {
    32: 32, 48: 48, 64: 64, 80: 96, 96: 96,
    128: 128, 160: 160, 192: 192, 256: 256,
}
...
def check_gfx9_tile_constraints(
    dtype: str, hdim_q: int, hdim_v: int, pipeline: str,
    bm0: int, bn0: int, bk0: int,
) -> bool:
    """Gfx9 compatibility rules (matches CompatibilityRuleFactoryGfx9).
    ...
    if (hdim_q, hdim_v) == (128, 128) and pipeline == "qr_async" and bm0 != 128:
        return False
    if (hdim_q, hdim_v) != (128, 128) and bm0 not in (128, 256):
        return False
...
def check_gfx950_tile_constraints(
    hdim_q: int, hdim_v: int, pipeline: str, bm0: int, bn0: int,
) -> bool:
    """Gfx950 trload/v3 constraints (matches CompatibilityRuleFactoryGfx950)."""
...
def check_qr_mfma_insts(
    arch: str, hdim_q: int, pipeline: str,
    bn0: int, bk0: int, wn0: int, wk0: int,
) -> bool:
    """NumMfmaInsts % 8 == 0 check (block_fmha_pipeline_qr_ks_vs.hpp:354).
...
def tile_passes_all_constraints(
    arch: str, dtype: str, hdim_q: int, hdim_v: int, pipeline: str,
    bm0: int, bn0: int, bk0: int, wm0: int, wn0: int, wk0: int,
) -> bool:
    """Master constraint check — returns True if the tile is valid."""
    elem_size = ELEMENT_SIZES.get(dtype, 2)
    lds_limit = LDS_LIMITS.get(pipeline, 32768)
    ...
    if arch in ("gfx90a", "gfx942", "gfx950"):
        if not check_gfx9_tile_constraints(dtype, hdim_q, hdim_v, pipeline, bm0, bn0, bk0):
            return False
    if arch == "gfx950":
...
# Dtype groups matching CK's classification
...
BWD_DQ_DK_DV_EXTRA_TILES: Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str, bool]]] = {
    (64, 64): [
...
def normalize_profile(profile: Optional[str] = None, receipt: Optional[str] = None) -> str:
    if profile:
...
def get_profile(profile: Optional[str] = None, receipt: Optional[str] = None) -> FmhaProfile:
...
def profile_allows(config: dict, profile: Optional[str] = None, receipt: Optional[str] = None) -> bool:
...
def validate_config(config: dict, arch_specs: Optional[dict] = None) -> "ValidationResult":
    ...
    # Family validation
    supported_families = {
        "fwd", "fwd_pagedkv", "fwd_splitkv", "fwd_splitkv_combine",
        "fwd_appendkv", "batch_prefill",
        "bwd_dot_do_o", "bwd_dq_dk_dv", "bwd_convert_dq",
    }
    ...
    if family.startswith("bwd") and dtype not in BWD_DTYPE_MAP:
        result.add_error(f"Backward family {family} only supports {sorted(BWD_DTYPE_MAP)}")

    if family.startswith("fwd") and not family.startswith("fwd_append") and dtype not in FWD_DTYPE_MAP:
        result.add_error(f"Forward family {family} does not recognize dtype {dtype}")

    # Pipeline validation
    if family != "fwd_splitkv_combine" and pipeline not in arch_info["supported_pipelines"]:
        result.add_error(f"pipeline {pipeline} is not supported on {arch}")

    if pipeline in {"v3", "qr_async_trload_v3"} and not arch_info.get("supports_v3", False):
    ...
    if global_rules.get("hdim_192_128_no_bias_dropout"):
        if hdim_q == 192 and hdim_v == 128 and (bias != "no" or sig.get("dropout", False)):
            result.add_warning("hdim (192,128) with bias/dropout has limited tile support")
    ...
    if len(tile) != expected_tile_len or len(alg["wave"]) != 9 or len(alg["warp"]) != 9:
        result.add_error(f"tile/wave/warp must have {expected_tile_len}/9/9 elements for {family}")

    # MFMA instruction count check for qr/h256/CDNA
    _1d_families = {"bwd_dot_do_o", "bwd_convert_dq"}
    if (
        pipeline == "qr" and hdim_q == 256
        and family not in _1d_families
        and arch_info.get("family", "").startswith("cdna")
        and len(tile) >= 3 and len(alg["wave"]) >= 2 and len(alg["warp"]) >= 3
    ):
