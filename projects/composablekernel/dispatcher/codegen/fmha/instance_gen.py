...
Consolidates tile generation (formerly fmha_tile_rules.py), feature-product
enumeration (formerly fmha_pipeline_rules.py), and instance expansion
(formerly fmha_instance_enum.py) into one clean module.

All hardware facts and constraints come from fmha_specs.py.
All symbol mappings come from fmha_symbol_map.py.

Usage:
    python fmha_instance_gen.py configs/receipt0_fwd.json --arch gfx950
    python fmha_instance_gen.py configs/fwd_ci.json --arch gfx950 --list
"""
...
def derive_bk1_fp8(bm0: int, bn0: int, bk0: int, hdim_q: int, hdim_v: int) -> int:
    """Derive bk1 for fp8 dtypes — always bk0."""
    return bk0
...
def generate_fwd_tiles(
    arch: str, dtype: str, hdim_q: int, hdim_v: int, pipeline: str = "qr_async",
) -> List[FmhaTileConfig]:
    """Generate ALL valid fwd tile configurations for given parameters."""
        ...
        for bn0 in VALID_BN0:
            for bk0 in VALID_BK0:
                for wm0, wn0, wk0 in warp_classes:
                    if not tile_passes_all_constraints(
                        arch, dtype, hdim_q, hdim_v, pipeline,
                    ...
                    tiles.append(FmhaTileConfig(
                        bm0=bm0, bn0=bn0, bk0=bk0,
                        bn1=hdim_v, bk1=bk1, bk0max=bk0max,
                        rm0=rm0, rm1=rm0,
                        wm0=wm0, wn0=wn0, wk0=wk0,
                        wm1=wm0, wn1=wn0, wk1=wk0,
                    ))

    return tiles


def generate_fwd_tiles_all_pipelines(
    arch: str, dtype: str, hdim_q: int, hdim_v: int,
) -> List[FmhaTileConfig]:
    """Generate tiles across all supported pipelines for an arch/dtype."""
    seen: Set[Tuple] = set()
    tiles: List[FmhaTileConfig] = []

    pipelines = ["qr", "qr_async"]
    if arch == "gfx950" and dtype in ("fp16", "bf16"):
        pipelines += ["qr_async_trload", "qr_async_trload_v3"]

    for pipeline in pipelines:
        for tile in generate_fwd_tiles(arch, dtype, hdim_q, hdim_v, pipeline):
            key = tile.tile_6 + (tile.wm0, tile.wn0, tile.wk0)
            if key not in seen:
                seen.add(key)
                tiles.append(tile)
...
def generate_splitkv_tiles(
    ...
    """Generate valid splitkv tiles. Uses fixed warp class per dtype."""
    bk0max = K0_MAX_SUBMAX_MAP.get(hdim_q, hdim_q)
...
def generate_pagedkv_tiles(
    ...
    return generate_splitkv_tiles(arch, dtype, hdim_q, hdim_v)


def generate_bwd_tiles(
    arch: str, dtype: str, hdim_q: int, hdim_v: int,
) -> List[FmhaTileConfig]:
    """Generate valid BWD tile configurations."""
    elem_size = ELEMENT_SIZES.get(dtype, 2)
...
def _fwd_specs_fp16bf16(
    hdim: int, hdim_v: int, receipt: int,
) -> List[PipelineSpec]:
    """Pipeline specs for fp16/bf16 on gfx9/gfx950."""
    specs: List[PipelineSpec] = []

    for logits, mask, bias, lse, dropout, skip, sink in itertools.product(
        BOOLS, MASKS, BIASES, BOOLS, BOOLS, BOOLS, BOOLS,
    ):
        if hdim == 256 and hdim_v == 256:
            specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="f", dpad="f", dvpad="f"))
            specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="t", dpad="f", dvpad="f"))
            specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="t", dpad="t", dvpad="t"))
        else:
            if bias == "bias":
                specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="f", dpad="f", dvpad="f"))
                specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="t", dpad="t", dvpad="t"))
            else:
                specs.append(PipelineSpec("qr_async", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="f", dpad="t", dvpad="t"))
                specs.append(PipelineSpec("qr_async", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="t", dpad="t", dvpad="t"))
...
def _fwd_specs_gfx950_extra(hdim: int, hdim_v: int) -> List[PipelineSpec]:
    ...
    for logits, mask, bias, lse, dropout, skip, sink in itertools.product(
        ...
        if (
            ...
            and logits == "f" and bias == "no" and dropout == "f" and skip == "f"
        ):
            specs.append(PipelineSpec("qr_async_trload", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="f", dpad="f", dvpad="f"))
            specs.append(PipelineSpec("qr_async_trload", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="f", dpad="t", dvpad="t"))

    if (hdim, hdim_v) == (128, 128):
        for logits, mask in itertools.product(BOOLS, ["no", "causal"]):
            specs.append(PipelineSpec("qr_async_trload_v3", mask, "no", "f", "f", logits, "f", "f", spad="t", skpad="t", dpad="f", dvpad="f"))
...
def _fwd_specs_fp8(hdim: int, hdim_v: int) -> List[PipelineSpec]:
    ...
    for logits, qscale, mask, bias, sink in itertools.product(
        BOOLS, ["no", "pertensor", "blockscale"], MASKS, ["no"], BOOLS,
    ):
        tag = "qr" if hdim == 64 else "qr_async"
        specs.append(PipelineSpec(tag, mask, bias, "f", "f", logits, "f", sink, qscale=qscale, spad="t", skpad="f", dpad="t", dvpad="t"))
        specs.append(PipelineSpec(tag, mask, bias, "f", "f", logits, "f", sink, qscale=qscale, spad="t", skpad="t", dpad="t", dvpad="t"))
...
def _fwd_specs_fp32(hdim: int, hdim_v: int) -> List[PipelineSpec]:
    ...
    for logits, mask, bias, lse, dropout, skip, sink in itertools.product(
        ...
        specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="f", dpad="f", dvpad="f"))
        specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="f", skpad="t", dpad="f", dvpad="f"))
        specs.append(PipelineSpec("qr", mask, bias, lse, dropout, logits, skip, sink, spad="t", skpad="t", dpad="t", dvpad="t"))
...
def get_pipelines_for_config(
    arch: str, dtype: str, hdim: int, hdim_v: int, receipt: int = 0,
) -> List[PipelineSpec]:
    ...
    return [s for s in specs if check_logits_bias(s.logits, s.bias) and receipt_filter(receipt, dtype, s)]


# --- SplitKV ---

def get_splitkv_pipelines(dtype: str, hdim: int, receipt: int = 0) -> List[SplitKVPipelineSpec]:
    """Split-KV main kernel pipelines."""
    specs: List[SplitKVPipelineSpec] = []
    SPLITKV_MASKS = ["no", "causal"]

    if dtype in DT_FP16_BF16:
        for logits, mask, bias, pagedkv, sink in itertools.product(BOOLS, SPLITKV_MASKS, BIASES, BOOLS, BOOLS):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, sink, pagedkv, spad="f", skpad="t", dpad="f", dvpad="f"))
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, sink, pagedkv, spad="t", skpad="f", dpad="f", dvpad="f"))
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, sink, pagedkv, spad="t", skpad="t", dpad="f", dvpad="f"))
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, sink, pagedkv, spad="t", skpad="t", dpad="t", dvpad="t"))
    elif dtype in ("fp8", "bf8"):
        for logits, mask, bias in itertools.product(BOOLS, SPLITKV_MASKS, BIASES):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, "f", "f", squant="t", spad="f", skpad="f", dpad="f", dvpad="f"))
            specs.append(SplitKVPipelineSpec("qr", mask, bias, logits, "f", "f", squant="t", spad="t", skpad="t", dpad="f", dvpad="f"))
...
def _splitkv_receipt_filter(receipt: int, dtype: str, spec: SplitKVPipelineSpec) -> bool:
    if receipt == 2:
        return dtype in ("fp16", "bf16") and spec.bias in ("no", "alibi") and spec.squant == "f" and spec.sink == "f"
    if receipt == 4:
        return dtype in ("fp16", "bf16") and spec.bias in ("no", "bias") and spec.squant == "f" and spec.sink == "f"
    if receipt == 200:
...
def get_splitkv_combine_pipelines(dtype: str, receipt: int = 0) -> List[SplitKVCombineSpec]:
...
def get_pagedkv_pipelines(dtype: str, hdim: int, receipt: int = 0) -> List[PipelineSpec]:
    ...
    if dtype in DT_FP16_BF16:
        for logits, mask, bias, sink in itertools.product(BOOLS, MASKS, BIASES, BOOLS):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(PipelineSpec("qr_pagedkv", mask, bias, "f", "f", logits, "f", sink, spad="t", skpad="f", dpad="f", dvpad="f"))
            specs.append(PipelineSpec("qr_pagedkv", mask, bias, "f", "f", logits, "f", sink, spad="t", skpad="t", dpad="f", dvpad="f"))
    elif dtype in ("fp8", "bf8"):
        for logits, mask, bias in itertools.product(BOOLS, MASKS, BIASES):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(PipelineSpec("qr_pagedkv", mask, bias, "f", "f", logits, "f", "f", spad="f", skpad="f", dpad="f", dvpad="f"))
            specs.append(PipelineSpec("qr_pagedkv", mask, bias, "f", "f", logits, "f", "f", spad="t", skpad="t", dpad="f", dvpad="f"))
...
def get_appendkv_pipelines(dtype: str, hdim: int, receipt: int = 0) -> List[AppendKVPipelineSpec]:
    """Append-KV pipelines."""
    specs: List[AppendKVPipelineSpec] = []

    if dtype in DT_FP16_BF16:
        for pagedkv in ["t", "f"]:
            specs.append(AppendKVPipelineSpec(rope="none", pagedkv=pagedkv, spad="f", skpad="t", dpad="f", dvpad="f"))
            specs.append(AppendKVPipelineSpec(rope="none", pagedkv=pagedkv, spad="t", skpad="t", dpad="t", dvpad="t"))
            specs.append(AppendKVPipelineSpec(rope="interleaved", pagedkv=pagedkv, spad="f", skpad="t", dpad="t", dvpad="f"))
            specs.append(AppendKVPipelineSpec(rope="interleaved", pagedkv=pagedkv, spad="t", skpad="t", dpad="t", dvpad="t"))
            specs.append(AppendKVPipelineSpec(rope="half_rotated", pagedkv=pagedkv, spad="f", skpad="t", dpad="t", dvpad="f"))
            specs.append(AppendKVPipelineSpec(rope="half_rotated", pagedkv=pagedkv, spad="t", skpad="t", dpad="t", dvpad="t"))
    elif dtype in ("fp8", "bf8"):
        specs.append(AppendKVPipelineSpec(rope="none", pagedkv="f", spad="t", skpad="t", dpad="t", dvpad="t"))
    return specs


# --- Batch Prefill ---

def get_batch_prefill_pipelines(dtype: str, hdim: int, receipt: int = 0) -> List[BatchPrefillPipelineSpec]:
    ...
    if dtype in DT_FP16_BF16:
        for logits, mask, bias, lse, dropout, kvl, kvt in itertools.product(
            BOOLS, PREFILL_MASKS, BIASES, BOOLS, BOOLS,
            ["vectorized", "linear"], ["vllm", "sglang"],
        ):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(BatchPrefillPipelineSpec(mask, bias, logits, "f", lse, dropout, "f", page_size=0, kv_memory_layout=kvl, kv_lookup_table=kvt))
    elif dtype == "fp8bf16":
        for logits, qscale, mask, bias, kvl, kvt in itertools.product(
            BOOLS, ["pertensor", "kv_blockscale"], MASKS, ["no"],
            ["vectorized", "linear"], ["vllm", "sglang"],
        ):
            if not check_logits_bias(logits, bias):
                continue
            specs.append(BatchPrefillPipelineSpec(mask, bias, logits, "f", "f", "f", "f", qscale=qscale, page_size=0, kv_memory_layout=kvl, kv_lookup_table=kvt))
    return specs
...
def get_bwd_dq_dk_dv_pipelines(dtype: str, receipt: int = 0) -> List[BwdPipelineSpec]:
    ...
    for mask, bias, dbias, dropout, deterministic in itertools.product(
        MASKS, BIASES, BOOLS, BWD_DROPOUTS, BOOLS,
    ):
        if bias != "bias" and dbias == "t":
            continue
        for dpad, dvpad in BWD_PAD_COMBOS:
            specs.append(BwdPipelineSpec("bwd_dq_dk_dv", mask, bias, dbias, dropout, deterministic, dpad=dpad, dvpad=dvpad))
    return specs


def get_bwd_dq_dk_dv_extra_pipelines(dtype: str, is_small: bool = False, receipt: int = 0) -> List[BwdPipelineSpec]:
    ...
    for mask, bias, dbias, dropout, deterministic in itertools.product(
        MASKS, BIASES, BOOLS, dropouts, BOOLS,
    ):
        if bias != "bias" and dbias == "t":
            continue
...
def get_bwd_dot_do_o_pipelines(dtype: str) -> List[BwdPipelineSpec]:
    ...
    if dtype not in DT_FP16_BF16:
        return []
    return [BwdPipelineSpec("bwd_dot_do_o", spad=s, dvpad=d) for s, d in itertools.product(BOOLS, BOOLS)]
...
def tile_compatible(
    ...
    pipeline_tag: str, tile: Tuple[int, ...],
) -> bool:
    """Check if a tile tuple is compatible (legacy interface for instance expansion)."""
    from specs import check_gfx9_tile_constraints, check_gfx950_tile_constraints, check_qr_mfma_insts
    bm0, bn0, bk0 = tile[0], tile[1], tile[2]

    if not check_gfx9_tile_constraints(dtype, hdim, hdim_v, pipeline_tag, bm0, bn0, bk0):
        return False
    if arch == "gfx950":
...
def bwd_dq_wave_warp(tile, hq, trload=False):
    ...
    if entry is None:
        ...
        return {
            "wave_m0": 1, "wave_n0": wn, "wave_k0": 1,
            "wave_m1": 4, "wave_n1": 1, "wave_k1": 1,
            "wave_m2": 1, "wave_n2": wn, "wave_k2": 1,
            "warp_m0": 16, "warp_n0": 16, "warp_k0": 32,
            "warp_m1": 16, "warp_n1": 16, "warp_k1": 16,
            "warp_m2": 16, "warp_n2": 16, "warp_k2": 16,
        }
    w = entry["wave"]
    wk1 = entry["warp_k1"]
    return {
        "wave_m0": w[0], "wave_n0": w[1], "wave_k0": w[2],
        "wave_m1": w[3], "wave_n1": w[4], "wave_k1": w[5],
        "wave_m2": w[6], "wave_n2": w[7], "wave_k2": w[8],
        "warp_m0": 16, "warp_n0": 16, "warp_k0": 32,
        "warp_m1": 16, "warp_n1": 16, "warp_k1": wk1,
        "warp_m2": 16, "warp_n2": 16, "warp_k2": 16,
    }
...
def expand_sweep(config_path: str, arch: str, receipt: int = 0) -> List[FmhaKernelConfig]:
    """Expand JSON sweep config into full kernel instance list.

    Tiles: constraint-based generation from fmha_specs.
    Features: pipeline-specific enumeration.
    JSON config: allow-list filter on the result.
    """
    ...
    dtypes = sorted(allowed_dtypes & arch_dtypes) if allowed_dtypes else sorted(arch_dtypes)
    ...
    if variant == "fwd":
        ...
        configs = _expand_appendkv(arch, dtypes, receipt)
    elif variant == "batch_prefill":
        configs = _expand_batch_prefill(arch, dtypes, receipt, allowed_masks, allowed_biases)
    elif variant == "bwd":
        configs = _expand_bwd(arch, dtypes, receipt, allowed_masks, allowed_biases, allowed_modes)
            ...
            for mode in MODES:
                ...
                for spec in pipeline_specs:
                    ...
                    if spec.tag not in _tile_cache:
                        _tile_cache[spec.tag] = generate_fwd_tiles(arch, dtype, hq, hv, spec.tag)
...
def _expand_splitkv(arch, dtypes, receipt, allowed_masks, allowed_biases, allowed_modes, allowed_logits=None, allowed_sink=None, allowed_paged_kv=None):
            ...
            for tc in tiles:
                for mode in MODES:
                    ...
                    for spec in sk_specs:
                        if mode == "group" and not (spec.spad == "t" and spec.skpad == "t"):
                            continue
                        mm = _MASK_MAP.get(spec.mask, spec.mask)
                        mb = _BIAS_MAP.get(spec.bias, spec.bias)
                        if allowed_masks is not None and mm not in allowed_masks:
    ...
    for dtype in dtypes:
        ...
        if not comb_specs:
            continue
        hdims = SPLITKV_COMBINE_HDIMS_FP16 if dtype in ("fp16", "bf16") else SPLITKV_COMBINE_HDIMS_FP8 if dtype in ("fp8", "bf8") else []
        for hv in hdims:
            for mode in MODES:
                if allowed_modes is not None and mode not in allowed_modes:
                    continue
                for spec in comb_specs:
                    if mode == "group" and spec.spad != "t":
                        continue
                    configs.append(FmhaKernelConfig(
                        family="fwd_splitkv_combine", data_type=dtype, mode=mode,
                        hdim_q=hv, hdim_v=hv, pipeline="splitkv_combine",
                        tile_m0=32, tile_n0=hv, tile_k0=32, tile_n1=32,
                        pad_s=_pad_val(spec.spad), pad_dv=_pad_val(spec.dvpad),
                        lse=(spec.lse == "t"), gfx_arch=arch,
                    ))
    return configs


def _expand_pagedkv(arch, dtypes, receipt, allowed_masks, allowed_biases, allowed_modes):
    configs = []
    for dtype in dtypes:
        for hq, hv in SUPPORTED_HDIMS.get(dtype, []):
                ...
                for mode in MODES:
                    ...
                    for spec in pk_specs:
                        ...
                        if allowed_biases is not None and mb not in allowed_biases:
                            continue
                        configs.append(FmhaKernelConfig(
                            family="fwd_pagedkv", data_type=dtype, mode=mode,
                            hdim_q=hq, hdim_v=hv, pipeline=spec.tag,
                            tile_m0=tc.bm0, tile_n0=tc.bn0, tile_k0=tc.bk0,
                            tile_n1=tc.bn1, tile_k1=tc.bk1, tile_k0max=tc.bk0max,
                            wave_m0=tc.rm0, wave_n0=1, wave_k0=1,
                            ...
                            pad_s=_pad_val(spec.spad), pad_sk=_pad_val(spec.skpad),
                            pad_d=_pad_val(spec.dpad), pad_dv=_pad_val(spec.dvpad),
                            mask=mm, bias=mb, logits=(spec.logits == "t"),
                            skip_min_seqlen_q=(spec.skip == "t"), sink=(spec.sink == "t"),
                            paged_kv=True, gfx_arch=arch,
                        ))
    return configs


def _expand_appendkv(arch, dtypes, receipt):
    configs = []
    for dtype in dtypes:
        ...
        for hq, hv in SUPPORTED_HDIMS.get(dtype, []):
            for spec in ak_specs:
                configs.append(FmhaKernelConfig(
                    family="fwd_appendkv", data_type=dtype, mode="batch",
                    hdim_q=hq, hdim_v=hv, pipeline="appendkv",
                    tile_m0=64, tile_n0=64, tile_k0=hq, tile_n1=hv,
                    pad_s=_pad_val(spec.spad), pad_sk=_pad_val(spec.skpad),
                    pad_d=_pad_val(spec.dpad), pad_dv=_pad_val(spec.dvpad),
                    rope={"none": "none", "interleaved": "interleaved", "half_rotated": "half_rotated"}.get(spec.rope, spec.rope),
                    paged_kv=(spec.pagedkv == "t"), gfx_arch=arch,
                ))
    return configs


def _expand_batch_prefill(arch, dtypes, receipt, allowed_masks, allowed_biases):
    configs = []
                ...
                for spec in bp_specs:
                    ...
                    for ps in page_sizes:
                        ...
                        configs.append(FmhaKernelConfig(
                            ...
                            hdim_q=hq, hdim_v=hv, pipeline="qr_async",
                            tile_m0=tc.bm0, tile_n0=tc.bn0, tile_k0=tc.bk0,
                            tile_n1=tc.bn1, tile_k1=bk1, tile_k0max=tc.bk0max,
                            wave_m0=tc.rm0, wave_n0=1, wave_k0=1,
                            wave_m1=tc.rm0, wave_n1=1, wave_k1=1,
                            warp_m0=tc.wm0, warp_n0=tc.wn0, warp_k0=tc.wk0,
                            warp_m1=tc.wm1, warp_n1=tc.wn1, warp_k1=tc.wk1,
                            pad_s=1, pad_sk=1, pad_d=1, pad_dv=1,
                            mask=mm, bias=mb, lse=(spec.lse == "t"),
                            dropout=(spec.dropout == "t"), logits=(spec.logits == "t"),
                            paged_kv=True, page_size=ps,
                            kv_memory_layout=spec.kv_memory_layout,
                            kv_lookup_table=spec.kv_lookup_table,
                            qscale=spec.qscale, gfx_arch=arch,
                        ))
    return configs


def _expand_bwd(arch, dtypes, receipt, allowed_masks, allowed_biases, allowed_modes):
    configs = []
    for dtype in dtypes:
            ...
            for mode in MODES:
                ...
                for spec in dot_specs:
                    if mode == "group" and spec.spad != "t":
                        continue
                    configs.append(FmhaKernelConfig(
                        family="bwd_dot_do_o", data_type=dtype, mode=mode,
                        hdim_q=hd, hdim_v=hd, pipeline="qr", tile_m0=64,
                        pad_s=_pad_val(spec.spad), pad_dv=_pad_val(spec.dvpad),
                        gfx_arch=arch,
                    ))
        ...
        for (hq, hv), tile in sorted(BWD_DQ_DK_DV_TILES_FP16.items()):
            for mode in MODES:
                ...
                for spec in dq_specs:
                    ...
                    configs.append(FmhaKernelConfig(
                        family="bwd_dq_dk_dv", data_type=dtype, mode=mode,
                        hdim_q=hq, hdim_v=hv, pipeline="qr",
                        tile_m0=tile[0], tile_n0=tile[1], tile_k0=tile[2],
                        ...
                        pad_s=_pad_val(spec.spad), pad_sk=_pad_val(spec.skpad),
                        pad_d=_pad_val(spec.dpad), pad_dv=_pad_val(spec.dvpad),
                        mask=mm, bias=mb, dbias=(spec.dbias == "t"),
                        dropout=(spec.dropout != "no"), dropout_variant=spec.dropout,
        ...
        for (hq, hv), extra_entries in BWD_DQ_DK_DV_EXTRA_TILES.items():
            for tile, tag, is_batch_only in extra_entries:
                dq_extra_specs = get_bwd_dq_dk_dv_extra_pipelines(dtype, is_small=is_batch_only, receipt=receipt)
                for mode in ["batch"] if is_batch_only else MODES:
                    ...
                    for spec in dq_extra_specs:
                        ...
                        configs.append(FmhaKernelConfig(
                            ...
                            tile_bwd7=tile[7] if len(tile) > 7 else 0,
                            tile_bwd8=tile[8] if len(tile) > 8 else 0,
                            tile_tag=tag, use_trload=(tag == "trload"),
                            pad_s=_pad_val(spec.spad), pad_sk=_pad_val(spec.skpad),
                            pad_d=_pad_val(spec.dpad), pad_dv=_pad_val(spec.dvpad),
                            mask=mm, bias=mb, dbias=(spec.dbias == "t"),
                            dropout=(spec.dropout != "no"), dropout_variant=spec.dropout,
            ...
            for mode in MODES:
                ...
                for spec in cvt_specs:
                    ...
                    for tile_grp in range(n_tile_groups):
                        configs.append(FmhaKernelConfig(
                            family="bwd_convert_dq", data_type=dtype, mode=mode,
                            hdim_q=hd, hdim_v=hd, pipeline="qr", tile_m0=64,
                            tile_tag=f"g{tile_grp}" if tile_grp > 0 else "",
                            pad_s=_pad_val(spec.spad), pad_d=_pad_val(spec.dpad),
                            deterministic=(spec.deterministic == "t"),
                            gfx_arch=arch,
                        ))
    return configs
...
# =============================================================================
# Exhaustive (oracle) tile enumeration
# =============================================================================


def enumerate_all_tiles_brute_force(
    arch: str, dtype: str, hdim_q: int, hdim_v: int, pipeline: str = "qr",
) -> List[Tuple[FmhaTileConfig, FmhaKernelConfig]]:
    """Generate ALL possible tile combinations without constraint filtering.

    Only applies the absolute minimum: alignment to warp shape and bk0 <= hdim_q.
    This is the "oracle" approach — let the compiler decide what's valid.
    """
    warp_classes = WARP_CLASSES.get(dtype, [(32, 32, 16)])
    is_fp8 = dtype in ("fp8", "fp8bf16", "fp8fp32", "bf8")
    bk0max = K0_MAX_SUBMAX_MAP.get(hdim_q, hdim_q)
...
def main():
    ...
    print(f"Expanded {args.config} -> {before} configs"
